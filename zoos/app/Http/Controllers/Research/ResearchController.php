...
use App\Http\Requests\Research\ResearchMaster\AddValidation;
use App\Http\Requests\Research\ResearchMaster\EditValidation;
...
use URL;
use ViewHelper;
class ResearchController extends CollegeBaseController
{
    protected $base_route = 'research.manage';
    protected $view_path = 'research.manage';
    protected $panel = 'Research';
    protected $folder_path;
    protected $folder_name = 'research';
    ...
    {
        $data = [];
        $data['books'] = ResearchMaster::select('id','code', 'title', 'image', 'categories', 'author', 'publisher', 'status')
    ...
    public function add(Request $request)
    {
        ...
        foreach (ResearchCategory::select('id', 'title')->get() as $category) {
            $data['categories'][$category->id] = $category->title;
        }

        $data['research_status'] = [];
        $data['research_status'][0] = 'Select Status';
        foreach (ResearchStatus::select('id', 'title')->get() as $book_status) {
            $data['research_status'][$book_status->id] = $book_status->title;
        }
    ...
    public function store(AddValidation $request)
    {
        ...
        $row = ResearchMaster::create($request->all());
        ...
        if ($row && $quantity > 0) {
            ...
            while ($i <= (int)$request->get('end')){
                Research::create([
                    'research_masters_id' => $row->id,
                    'book_code' => $request->get('code').$i,
                    'created_by' => auth()->user()->id,
                    'research_status' => $request->get('research_status')?$request->get('research_status'):1,
                ]);
                $i++;
            }
        }
    ...
    {
        $data = [];
        $data['row'] = ResearchMaster::select('id',  'isbn_number', 'code', 'title', 'sub_title', 'image',
    ...
    {
        if (!$row = ResearchMaster::find($id)) return parent::invalidRequest();
    ...
    {
        $data = [];
        $data['books'] = ResearchMaster::select('id', 'isbn_number', 'code', 'title', 'sub_title', 'image',
            'language', 'editor', 'categories', 'edition', 'edition_year', 'publisher', 'publish_year', 'series', 'author',
            'rack_location', 'price', 'total_pages', 'source', 'notes', 'status')
            ->where('id','=',$id)
            ->orderBy('title','asc')
        ...
        $data['books_collection'] = Research::where('research_masters_id','=',$data['books']->id )
            ->get();

        $data['books_status'] = ResearchStatus::select('id', 'title', 'display_class')->get();
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    switch ($request->get('bulk_action')) {
                        case 'active':
                        case 'in-active':
                            $row = ResearchMaster::find($row_id);
                            if ($row) {
                        ...
                        case 'delete':
                            ...
                            /*delete books copies*/
                                $copies = Research::where('research_masters_id','=',$row->id)->get();
                                if($copies){
                                    foreach($copies as $copy){
                                        $copy->delete();
                                    }
    ...
    {
        if (!$row = Research::find($id)) return parent::invalidRequest();

        $request->request->add(['research_status' => $status]);
    ...
    public function addCopies(Request $request)
    {
        ...
        if($start > $end){
            $request->session()->flash($this->message_warning, $this->panel. ' Attention!, You have enter End Value is Less than Start. Correct It.');
            return back();
        }
    ...
    public function bulkCopiesDelete(Request $request)
    {
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['delete'])) {
            if ($request->has('chkIds')) {
                ...
                $request->session()->flash($this->message_warning, 'Please, Check at least One Chemical.');
                return redirect()->route($this->base_route);
            }
    ...
    public function handleImportBook(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            //category id
            $category = ResearchCategory::where('title',$row['category'])->first();
            if($category){
            ...
            $validator = Validator::make($row, [
                'code'              => 'required | max:25 | unique:research_masters,code',
                'start'             => 'required | min:1',
                'end'               => 'required | min:1',
                'title'             => 'required | max:100 | unique:research_masters,title',
            ...
            if ($row && $row['start'] > 0 && $row['end'] > 0) {
                ...
                while ($i <= (int)$row['end']){
                    Research::create([
                        'research_masters_id' => $book->id,
                        'research_code' => $row['code'].$i,
                        'created_by' => auth()->user()->id,
                    ]);
                    $i++;
                }
        ...
        $request->session()->flash($this->message_success,'Chemical imported Successfully');
        return redirect()->route($this->base_route);
