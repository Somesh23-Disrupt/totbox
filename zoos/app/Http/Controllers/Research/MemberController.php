...
use App\Http\Requests\Research\Member\AddValidation;
use App\Http\Requests\Research\Member\EditValidation;
...
{
    protected $base_route = 'research.member';
    protected $view_path = 'research.member';
    ...
    {
        $data = [];
        $data['member'] = ResearchMember::select('id', 'user_type', 'member_id', 'status')
        ...
        foreach (ResearchCirculation::select('id', 'user_type')->get() as $circulation) {
    ...
    {
        $data = [];
        $data['member'] = ResearchMember::select('id', 'user_type', 'member_id', 'status')->get();
    ...
    public function store(AddValidation $request)
    {
        ...
        if($data){
            $currentMember = ResearchMember::where(['user_type' => $request->user_type, 'member_id' => $data->id])->orderBy('id','desc')->first();
            if(!$currentMember){
                $request->request->add(['member_id' => $data->id]);
                $request->request->add(['created_by' => auth()->user()->id]);
                $member = ResearchMember::create($request->all());
    ...
    public function quickMembership(Request $request)
    {
        ...
        if($data){
            ...
            if(!$currentMember){
                ...
                $request->session()->flash($this->message_success, 'Research Member Created Successfully.');
            }else{
    ...
    {
        $data = [];
        if (!$data['row'] = ResearchMember::find($id))
            return parent::invalidRequest();
    ...
    {
        if (!$row = ResearchMember::find($id)) return parent::invalidRequest();
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    switch ($request->get('bulk_action')) {
                        case 'active':
                        case 'in-active':
                            $row = ResearchMember::find($row_id);
                            if ($row) {
    ...
    public function inActive(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->semester.' '.$this->panel.' In-Active.');
        return redirect()->route($this->base_route);
    }
        ...
        if(!$alert) {
            ...
            if($userType == 1){
                $student = Student::select('students.id', 'students.first_name','students.email', 'ai.mobile_1')
                    ...
                    ->find($memberId);
                //send alert
                //Dear {{first_name}}, Congratulation! You are successfully registered in our Research team.
            ...
            if($userType == 2){
                $staff = Staff::select('first_name','email', 'mobile_1')
                    ->find($memberId);
                //send alert
                //Dear {{first_name}}, Congratulations! You are successfully registered in our library.
                $subject = $alert->subject;
                $message = $alert->template;
