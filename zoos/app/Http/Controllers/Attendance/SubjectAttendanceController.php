...
class SubjectAttendanceController extends CollegeBaseController
{
    ...
    public function findSubject(Request $request)
    {
        ...
        //$subjects = array_pluck($subjects, 'title','id');

        if ($subjects->count() > 0) {

            $response['subjects'] = $subjects;
            $response['success'] = 'Sem./Sec. Subject Found, Choose For Manage Subject Wise Attendance.';
        }else {
            $response['error'] = 'No Any Subject Map in Sem/Sec. Please Assign Subject on Sem./Sec. First.';
        }
    ...
    public function studentHtmlRow(Request $request)
    {
        ...
        /*
         *  "_token" => "UZ09xQhKmTi4YaTg5vfVXj6TDYoSrkepPj43657A"
              "date" => "2019-05-11"
              "faculty_id" => "1"
              "semester_id" => "1"
              "sem_subject" => "1"
        ...
        $attendanceExist = SubjectAttendance::select('subject_attendances.link_id','subject_attendances.subjects_id','subject_attendances.attendance_type',
            'subject_attendances.years_id','subject_attendances.months_id','subject_attendances.'.$day,
            's.id as students_id','s.reg_no','s.first_name','s.middle_name','s.last_name','s.student_image')
            ->where('subject_attendances.subjects_id',$subject)
            ->where('subject_attendances.attendance_type',$attendance_type)
