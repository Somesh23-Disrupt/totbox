@foreach($schedules as $subject)
    <tr class="option_value" style="background: lightgrey">
        ...
        <td>
            {!! Form::text('date[]', \Carbon\Carbon::parse($subject->date)->format('Y-m-d'), ["placeholder" => "YYYY-MM-DD", "class" => "input-sm form-control border-form input-mask-date date-picker", "data-date-format" => "yyyy-mm-dd", "required"]) !!}
        </td>
        <td>
