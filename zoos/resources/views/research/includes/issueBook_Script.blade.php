...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('select[name="book_id"]').select2({
            ...
            ajax: {
                url: '{{ route('research.book-name-autocomplete') }}',
                dataType: 'json',
                delay: 250,
        ...
        $('#load-html-btn').click(function () {
            ...
            if (!book_id)
                toastr.warning("Please, Choose Chemical.", "Warning");
            else {

                $.ajax({
                    type: 'POST',
                    url: '{{ route('research.book-detail-html') }}',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: book_id
                    },
                    success: function (response) {
                        ...
                        if (data.error) {
                            ...
                            toastr.success('Please Select Chemical', 'Success:')
                        }
                    }
                });
