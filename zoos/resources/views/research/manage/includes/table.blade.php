                ...
                @if (isset($data['books']) && $data['books']->count() > 0)
                    ...
                    @foreach($data['books'] as $books)
                        <tr>
                            ...
                            <td>{{ $i }}</td>
                            <td>
                                <a href="{{ route('research.manage') }}?categories={{$books->categories}}">
                            ...
                            <td>
                                @if($books->image)
                                    <a href="{{ route($base_route.'.view', ['id' => $books->id]) }}">
                                        <img src="{{ asset('images'.DIRECTORY_SEPARATOR.'manage'.DIRECTORY_SEPARATOR.$books->image) }}"
                            ...
                            <td>
                                <a href="{{ route('research.manage') }}?author={{$books->author}}">
                                    {{ $books->author }}
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('research.manage') }}?publisher={{$books->publisher}}">
                                    {{ $books->publisher }}
                                </a>
                            </td>
                            <td>{{ $books->bookCollection()->count() }} </td>
                            <td>{{ $books->bookCollection()->where('research_status','=',2)->count() }} </td>
                            <td>{{ $books->bookCollection()->where('research_status','=',1)->count() }} </td>
                            <td class="hidderesearch0 ">
                                <div class="btn-group">
