<div class="row">
    <div class="col-xs-12">
        <h4 class="header large lighter black"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;Staff Chemical Return Over Period List</h4>
                ...
                <tbody>
                    @if (isset($data['staff_return_over']) && $data['staff_return_over']->count() > 0)
                        ...
                        @foreach($data['staff_return_over'] as $staff_return_over)
                            <tr>
                                <td>{{ $i }}</td>
                                <td><a href="{{ route('research.staff.view', ['id' => $staff_return_over->member_id]) }}"> {{ ViewHelper::getStaffById($staff_return_over->member_id) }} </a></td>
                                ...
                                <td><a href="{{ route('research.book.view', ['id' => $staff_return_over->bookmaster_id]) }}">{{ $staff_return_over->title }}</a> </td>
