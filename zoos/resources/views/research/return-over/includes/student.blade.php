<div class="row">
    <div class="col-xs-12">
        <h4 class="header large lighter black"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;Student Chemical Return Over Period List</h4>
            ...
            <table id="" class="table table-striped table-bordered table-hover">
                ...
                @if (isset($data['student_return_over']) && $data['student_return_over']->count() > 0)
                    ...
                    @foreach($data['student_return_over'] as $student_return_over)
                        <tr>
                            <td>{{ $i }}</td>
                            <td><a href="{{ route('research.student.view', ['id' => $student_return_over->member_id]) }}"> {{ ViewHelper::getStudentById($student_return_over->member_id) }} </a></td>
                            ...
                            <td><a href="{{ route('research.book.view', ['id' => $student_return_over->bookmaster_id]) }}">{{ $student_return_over->title }}</a> </td>
