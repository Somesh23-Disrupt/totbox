<div class="row">
    <div class="col-xs-12">
        <h4 class="header large lighter black"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;Book Taken List</h4>
        <div class="clearfix">
                ...
                @if (isset($data['books_taken']) && $data['books_taken']->count() > 0)
                    ...
                    @foreach($data['books_taken'] as $book_taken)
                        <tr>
                            ...
                            <td>
                                @if($book_taken->image)
                                    <img id="avatar" class="editable img-responsive" alt="{{ $book_taken->title }}" src="{{ asset('images'.DIRECTORY_SEPARATOR.'manage'.DIRECTORY_SEPARATOR.$book_taken->image) }}" style="width: 35px;" />
                                @endif
                            </td>
                            ...
                            <td>
                                <a href="{{ route('research.return', ['id' => $book_taken->book_id, 'member_id' => $data['student']->id]) }}" class="btn btn-xs btn-primary">
