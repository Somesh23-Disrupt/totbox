<div class="row">
    <div class="col-xs-12">
        {{--<h4 class="header large lighter black"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;Subject Attendance Detail</h4>--}}
        ...
        <div class="table-responsive">
            <table id="dynamic-table-1" class="table table-striped table-bordered table-hover">
                ...
                @if (isset($data['subject_attendance']) && $data['subject_attendance']->count() > 0)
                    @php($i=1)
                    @foreach($data['subject_attendance'] as $attendance)
                        <tr>
                            <td>{{ $i }}</td>
                           <td>{{ ViewHelper::getYearById($attendance->years_id) }} </td>
                            <td>{{ ViewHelper::getMonthById($attendance->months_id) }} </td>
                            <td>{{ ViewHelper::getSubjectById($attendance->subjects_id) }} </td>
                            <td>
                                {{ ($attendance->attendance_type == 1)?"Theory":"Practical" }}
                            </td>
                ...
                @else
                    <tr>
                        <td colspan="47"  class="align-left">No attendance data found.</td>
                    </tr>
                @endif
                </tbody>
            </table>
